...
const TIMES_SHORT = ['AM', 'PM', 'Eve', 'Night']
...
export function CadenceHeatmap({ grid }: CadenceHeatmapProps) {
  ...
  return (
    <div
      className="rounded-card p-4 sm:p-6 flex flex-col gap-4"
    ...
    >
      ...
      {/* Legend */}
      <div className="flex items-center gap-1.5 sm:gap-2 pt-2 flex-wrap" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <span style={{ fontSize: '10px', color: '#4A4A62' }}>Low</span>
        {[0.03, 0.08, 0.18, 0.35, 0.55].map((op, i) => (
          <div key={i} className="w-4 sm:w-5 h-3 rounded-sm" style={{ background: `rgba(255,255,255,${op})`, border: '1px solid rgba(255,255,255,0.06)' }} />
        ))}
        <span style={{ fontSize: '10px', color: '#4A4A62' }}>High</span>
      </div>
    </div>
  )
}
